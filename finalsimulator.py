binary_file = "/Users/shanthu/Downloads/final_valuation_framework_mar30_2025_students_v5/automatedTesting/tests/bin/simple/simple_6.txt"
...
def outputfile(output_file,PC,registers):
    if PC == 0:
      open(output_file, "w").close()
    else:
      with open(output_file,"a") as f:
          f.write(str(PC)+" ")
          f.write(" ".join(str(registers[i]) for i in sorted(registers.keys())) + "\n")

def outputmemory(output_file,memory_allocation):
    with open(output_file, "a") as f:
...
if __name__ == "__main__":
    ...
    for line in binaryfile:
        ...
        outputfile(output_file, PC, registers)
    outputmemory(output_file, memory_allocation)